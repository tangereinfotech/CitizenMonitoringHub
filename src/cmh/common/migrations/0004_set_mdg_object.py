...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        mdgs = [None] # So that we can index from 1
        for x in range (1, 9):
            mdg = orm ['common.MilleniumDevGoal'].objects.create (goalnum = str (x))
            mdgs.append (mdg)

        for cmdg in orm ['common.ComplaintMDG'].objects.all ():
            goalnums = cmdg.goalnum.split (',')
            for goalnum in goalnums:
                cmdg.mdg = mdgs [int (float (goalnum))]
                cmdg.save ()
    ...
    def backwards(self, orm):
        ...
        orm ['common.MilleniumDevGoal'].objects.all ().delete ()
